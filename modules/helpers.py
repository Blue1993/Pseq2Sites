...
def get_results(binding_sites, pred_binding_sites, sequences):
    ...
    for bs, bps, seq in zip(binding_sites, pred_binding_sites, sequences):
        ...
        positive_label = bs.split(",")
        negative_label = list(set(index) - set(positive_label))
        positive_pred = bps.split(",")
        negative_pred = list(set(index) - set(positive_pred))
        
        TP = len(set(positive_pred) & set(positive_label))
        TN = len(set(negative_pred) & set(negative_label))
        FP = len(set(positive_pred) & set(negative_label))
        FN = len(set(negative_pred) & set(positive_label)) 
    ...
    return precision, recall, specificity, ACC, G_mean, F1_score, F2_score
